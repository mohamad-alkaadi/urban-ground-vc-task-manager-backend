...
export async function processUserIntent(
  ...
  try {
    ...
    console.log(`Orchestration Layer Exception ${error.message}`);

    // Check if the error is due to Google API high demand (503 Service Unavailable)
    const isHighDemand =
    ...
    return {
      ...
      tasks: await taskAgent.getTasks().catch(() => []), // Safeguard to ensure tasks return even if DB fails
      updatedHistory: history, // Keep existing history intact so the conversation isn't wiped out
      error: true,
    };
  }
}
